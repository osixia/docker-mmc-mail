...
if [ ! -e "$FIRST_START_DONE" ]; then
  ...
  if [ "${MMC_MAIL_LDAP_CLIENT_TLS,,}" == "true" ]; then
    ...
    cfssl-helper ${LDAP_CLIENT_CFSSL_PREFIX} "${CONTAINER_SERVICE_DIR}/ldap-client/assets/certs/$MMC_MAIL_LDAP_CLIENT_TLS_CRT_FILENAME" "${CONTAINER_SERVICE_DIR}/ldap-client/assets/certs/$MMC_MAIL_LDAP_CLIENT_TLS_KEY_FILENAME" "${CONTAINER_SERVICE_DIR}/ldap-client/assets/certs/$MMC_MAIL_LDAP_CLIENT_TLS_CA_CRT_FILENAME"
